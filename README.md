...
插件处于快速迭代阶段，采用以下版本命名方式：

| 阶段 | 格式 | 含义 |
|------|------|------|
| 迭代版 | `v0.迭代号.0` | 开发中，功能可能调整，标记为 GitHub Pre-release |
| 正式版 | `v1.0.0` | 功能稳定后发布 |

每次迭代版本号 +1（如 `v0.1.0` → `v0.2.0` → `v0.3.0`），方便跟踪变更。

## 版本历史

- **v0.2.0** — 修复延迟过低问题：改用中位数算法，新增配置文件自定义测速 URL
- **v0.1.0** — 多站点测速 (Beta)：增加至 6 个测速端点，取最小延迟
